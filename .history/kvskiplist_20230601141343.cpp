...
void test_thread(thread_safe_skip_list<int, int> &list)
{
    ...
    {
        list.insert(dis(gen), dis(gen));
    }
}
// 多线程测试函数
void test_thread(thread_safe_skip_list<int, int> &list, unsigned int seed)
{
...
int main()
{
    // 高并发测试
    {
        ...
        {
            unsigned int seed = rd(); // 为每个线程生成一个唯一的种子
            threads.emplace_back(test_thread, std::ref(list), seed);
        }
        for (auto &t : threads)
        {
            t.join();
